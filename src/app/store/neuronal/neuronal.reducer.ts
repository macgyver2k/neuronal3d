...
function upsertEntryInCollection(
  col: NeuronalState['modelCollection'],
...
export const neuronalReducer = createReducer<NeuronalState>(
  ...
  on(
    ...
    (s, { modelCollection }): NeuronalState => ({
      ...s,
      modelCollection,
  ...
  on(NeuronalActions.modelEntryUpserted, (s, { entry }): NeuronalState => {
    ...
    return {
      ...
      epochDisplayRows: existed
        ? s.epochDisplayRows
        : epochRowsForId(s, entry.id),
    };
  }),
  on(
    NeuronalActions.epochViewSyncFromModel,
    (s, { modelId }): NeuronalState => {
      if (!modelId) {
  ...
  on(
    NeuronalActions.trainingFinished,
    (s, { lastTrainLoss, lastTrainBatchAcc }): NeuronalState => ({
      ...s,
      lastTrainLoss,
      lastTrainBatchAcc,
      training: {
  ...
  on(
    NeuronalActions.trainingStopRequested,
    (s): NeuronalState => ({
      ...s,
  ...
  on(
    NeuronalActions.trainingPauseToggled,
    (s): NeuronalState => ({
      ...s,
  ...
  on(NeuronalActions.uiModelDropdownToggleRequested, (s): NeuronalState => {
    ...
    if (!s.modelStoreHydrated || s.modelCollection.models.length === 0)
      return s;
    return { ...s, modelDropdownOpen: !s.modelDropdownOpen };
  }),
  on(
    NeuronalActions.activeModelFromToolbarRequested,
    (s): NeuronalState => ({
      ...s,
      modelDropdownOpen: false,
    }),
  ...
  on(
    NeuronalActions.modelDropdownSetOpen,
    (s, { open }): NeuronalState => ({
      ...s,
  ...
  on(
    NeuronalActions.vizInputLayerLayoutChanged,
    (s, { raw }): NeuronalState => {
  ...
  on(
    NeuronalActions.vizInputLayerScaleChanged,
    (s, { scale }): NeuronalState => {
  ...
  on(
    NeuronalActions.vizHiddenLayerLayoutChanged,
    (s, { index, raw }): NeuronalState => {
  ...
  on(
    NeuronalActions.vizHiddenLayerScaleChanged,
    (s, { index, scale }): NeuronalState => {
      if (!Number.isFinite(scale)) return s;
      const clamped = clampHiddenLayerVizScale(scale);
      const nextScales: [number, number] = [
        s.viz3d.hiddenLayerScales[0],
        s.viz3d.hiddenLayerScales[1],
      ];
  ...
  on(
    NeuronalActions.vizActiveNeuronMaxScaleMulChanged,
    (s, { mul }): NeuronalState => {
