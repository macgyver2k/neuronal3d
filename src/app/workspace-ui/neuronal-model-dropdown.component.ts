...
@Component({
  selector: 'app-neuronal-model-dropdown',
  standalone: true,
  imports: [NgClass],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    @if (showCaption(); as cap) {
      <span
    ...
    <div
      class="dropdown z-[60] w-full min-w-0"
                    ...
                    <div class="flex w-full flex-wrap gap-1">
                      ...
                      >
                        <span
                          ...
                          >Epoch</span
                        >
                        <span class="text-success text-sm font-semibold tabular-nums">{{
                          item.epochValue
                        }}</span>
                      </div>
                      <div
                      ...
                      >
                        ...
                        >
                        <span class="text-info text-sm font-semibold tabular-nums">{{
                          item.accValue
                        }}</span>
                      </div>
                      <div
                      ...
                      >
                        <span
                          ...
                          >Fehlerrate</span
                        >
                        <span class="text-warning text-sm font-semibold tabular-nums">{{
                          item.errValue
                        }}</span>
                      </div>
                    </div>
...
export class NeuronalModelDropdownComponent {
  private readonly store = inject(Store<AppState>);

  readonly showCaption = input<string | null>(null);

  @HostBinding('class')
  readonly hostClass = 'flex min-w-0 w-full flex-col gap-1.5';
