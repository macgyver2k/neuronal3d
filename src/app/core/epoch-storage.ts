...
export function normalizeEpochTrackStore(data: unknown): EpochTrackStore | null {
  if (!data || typeof data !== "object") return null;
  const p = data as Record<string, unknown>;
  if (p["version"] !== 1 || typeof p["byModelId"] !== "object" || p["byModelId"] === null) return null;
  const byModelId: Record<string, PersistedEpochRow[]> = {};
  for (const [id, arr] of Object.entries(p["byModelId"] as Record<string, unknown>)) {
...
export function loadEpochTrackStoreFromStorage(): EpochTrackStore {
  try {
    ...
    return normalizeEpochTrackStore(p) ?? { version: 1, byModelId: {} };
  } catch {
    return { version: 1, byModelId: {} };
  }
}
